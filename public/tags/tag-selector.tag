		...
		<div class="form-content grid-y">
			<div class="contents cell medium-auto" if={ !loading }>
				<label each={ item in items } class={ opts.fullWidth ? 'full-width' : '' }>
					<input type="checkbox" checked={ item.selected } onchange={ parent.elementChecked } if={ !opts.unique }>
					<input type="radio" name="selection" checked={ item.selected } onchange={ parent.elementChecked } if={ opts.unique }>
